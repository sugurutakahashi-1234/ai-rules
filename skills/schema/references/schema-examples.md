...
> Google retired FAQ rich results in May 2026. Use this example only when the page visibly contains genuine FAQs and a non-Google consumer or schema.org-level description justifies the markup; do not present it as a Google rich-result tactic.
...
> Google no longer provides a HowTo rich result. Use this example only for a documented non-Google consumer or a clear schema.org-level purpose; do not present it as a Google or AI-search visibility tactic.
